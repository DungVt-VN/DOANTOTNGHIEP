...
const StudentClasses = () => {
  ...
  // --- HELPER: Format Thứ (Xử lý số 8 -> Chủ Nhật) ---
  const formatDays = (daysStr) => {
    if (!daysStr) return "Chưa cập nhật";

    // Tách chuỗi ngày (ví dụ: "2,4,8") thành mảng
    ...
    const formatted = days.map((day) => {
      ...
      // Các trường hợp coi là Chủ Nhật
      if (["8", "cn", "sun", "sunday"].includes(d)) return "CN";
      return d;
    });

    // Nếu chỉ có 1 ngày và là CN -> "Chủ Nhật"
    if (formatted.length === 1 && formatted[0] === "CN") {
      return "Chủ Nhật";
    }

    // Nếu nhiều ngày hoặc ngày thường -> "Thứ 2, 4, CN"
    ...
    return (
      <div className="group bg-white rounded-2xl border border-blue-200 p-5 hover:shadow-xl hover:shadow-blue-50 transition-all duration-300 flex flex-col h-full relative">
        ...
        <div className="grid grid-cols-2 gap-3 mt-auto mb-5">
          {/* Thời gian - Đã sửa phần hiển thị Thứ */}
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex flex-col gap-1">
            <div className="flex items-center gap-2 text-slate-500 font-bold text-[11px] uppercase">
              <Calendar size={14} />
              {/* Gọi hàm formatDays thay vì hiển thị trực tiếp */}
